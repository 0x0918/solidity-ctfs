...
contract ConfidentialHash {
  ...
  bytes32 private ALICE_PRIVATE_KEY = bytes32(uint256(9)); // Super Secret Key, not known by attacker from code
  ...
  bytes32 private BOB_PRIVATE_KEY = bytes32(uint256(9)); // Super Secret Key, not known by attacker from code
